...
fun ItemEntryScreen(
    ...
    //呼び出されたコンポジションにバインドされた CoroutineScope を返すコンポーズ可能な関数
    //コンポーザブルの外部でコルーチンを開始し、スコープがコンポジションから離れた後にコルーチンがキャンセルされるようにする場合、
    // コンポーズ可能な関数 rememberCoroutineScope() を使用できます。
    // この関数は、コルーチンのライフサイクルを手動で制御する必要がある場合
    // （ユーザー イベントが発生するたびにアニメーションをキャンセルする場合など）に使用できます。
    ...
    Scaffold(
        ...
        ItemEntryBody(
            ...
            onItemValueChange = viewModel::updateUiState,// viewModelインスタンスのupdateUiStateの参照を渡している
            onSaveClick = {
                // キャンセル可能な非同期処理を行う
